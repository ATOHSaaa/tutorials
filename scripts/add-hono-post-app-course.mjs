...
import { HONO_POST_APP_COURSE } from './hono-post-app-course-data.mjs'

const SRC = path.resolve('src')
const TEMPLATE = path.join(SRC, 'cloudflare')
const PROPOSED_COURSES = [HONO_POST_APP_COURSE]
...
.hpa-arch { display: flex; align-items: center; justify-content: center; gap: 0.5rem; flex-wrap: wrap; padding: 1rem; background: var(--bg-3); border-radius: var(--radius-sm); margin: 0.75rem 0; }
.hpa-box { padding: 0.5rem 1rem; border: 2px solid var(--border); border-radius: var(--radius-sm); font-size: 0.85rem; font-weight: 600; }
.hpa-box.active { border-color: var(--accent); background: var(--accent-bg); color: var(--accent); }
.hpa-arrow { color: var(--text-muted); }
.hpa-post { padding: 0.75rem 1rem; background: var(--bg-3); border: 1px solid var(--border); border-radius: var(--radius-sm); margin-bottom: 0.5rem; font-size: 0.85rem; }
.hpa-post-meta { font-size: 0.75rem; color: var(--text-muted); margin-top: 0.35rem; display: flex; gap: 1rem; }
.hpa-like-btn { background: none; border: none; cursor: pointer; font-size: 0.85rem; padding: 0; }
.hpa-like-btn.liked { color: #e11d48; }
.hpa-route-list { display: flex; flex-direction: column; gap: 0.35rem; }
.hpa-route { font-family: var(--mono); font-size: 0.78rem; padding: 0.4rem 0.65rem; background: var(--bg-3); border-radius: var(--radius-sm); }
.hpa-route.get { border-left: 3px solid #22c55e; }
.hpa-route.post { border-left: 3px solid #3b82f6; }
.hpa-route.delete { border-left: 3px solid #ef4444; }
.hpa-follow-graph { display: flex; align-items: center; justify-content: center; gap: 1rem; padding: 1rem; }
.hpa-user-node { width: 64px; height: 64px; border-radius: 50%; background: var(--bg-3); border: 2px solid var(--border); display: flex; align-items: center; justify-content: center; font-size: 1.5rem; }
.hpa-user-node.following { border-color: var(--accent); }
`
...
for (const course of PROPOSED_COURSES) {
  ...
  appCss = appCss.replace(/\.cf-/g, '.hpa-legacy-')
  appCss += DEMO_CSS
...
const course = HONO_POST_APP_COURSE
